...
class Points(object):
    ...
    def __str__(self):
        ...
        return "<sba.Points() with {0} world points (R{2}) in {1} cameras (R{3})>".format(
            self.n,
            self.ncameras,
            self.pnp,
            self.mnp
    ...
    def _fromDylan(cls, dylan_array):
        ...
        ncameras = int((dylan_array.shape[1] - 3) / 2)

        x = np.zeros((npoints, ncameras, 2), dtype=np.double)
        vmask = np.zeros((npoints, ncameras), dtype=np.byte)
        for aa in range(npoints):
            for bb in range(ncameras):
    ...
    def toDylan(self):
        ...
        for eachrow in range(self.__n):
            for eachcol in range(self.__ncameras):
